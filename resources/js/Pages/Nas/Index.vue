    ...
    if (dataTable) {
     ...
     if (tableRef.value) {
        dataTable = new DataTable(tableRef.value, {
            ...
            // Ajusta el DOM para facilitar el estilo con Tailwind
            dom: '<"flex flex-col lg:flex-row items-center justify-between gap-4 mb-4"<"flex items-center gap-2"l><"flex items-center gap-2"f>><"overflow-x-auto"rt><"flex flex-col lg:flex-row items-center justify-between gap-4 mt-4"<"mb-4 lg:mb-0"i><"flex gap-2"p>>',

            // Aplica clases Tailwind usando DOM API (sin jQuery)
            initComplete: function() {
                console.log('DataTable initComplete called');

                const wrapper = tableRef.value && tableRef.value.closest && tableRef.value.closest('.dataTables_wrapper') || document;

                const addClassesTo = (root, selector, classString) => {
                    const classes = classString.split(/\s+/).filter(Boolean);
                    const nodes = root.querySelectorAll(selector);
                    console.log('apply', selector, 'found', nodes.length);
                    nodes.forEach(el => {
                ...
                // Campo de búsqueda: darle ancho adecuado
                const searchInputs = addClassesTo(wrapper, 'div.dataTables_filter input', 'px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none w-56');
                if (searchInputs.length === 0) {
                    // fallback: buscar globalmente
                    addClassesTo(document, 'input.dataTables_search', 'px-3 py-2 w-56');
                }

                addClassesTo(wrapper, 'div.dataTables_filter label', 'inline-flex items-center gap-2');

                // Control de longitud: alinear label y dar ancho fijo al select
                const lenSelects = addClassesTo(wrapper, 'div.dataTables_length select', 'px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none w-16 appearance-none bg-white');
                addClassesTo(wrapper, 'div.dataTables_length label', 'inline-flex items-center gap-2');
                if (lenSelects.length === 0) {
                    // intenta buscar select dentro del wrapper con otra estructura
                    const alt = wrapper.querySelectorAll('select');
                    alt.forEach(el => {
                ...
                // Paginación: botones espaciados y estilo del botón activo
                addClassesTo(wrapper, 'div.dataTables_paginate a.paginate_button', 'px-3 py-1.5 mx-0.5 rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors');
                wrapper.querySelectorAll('div.dataTables_paginate a.paginate_button.current').forEach(el => {
                    el.classList.add('bg-blue-600', 'text-white', 'border-blue-600', 'hover:bg-blue-700');
                });
            },
        ...
        <div class="bg-white p-6 rounded-lg shadow-lg border border-gray-100">
            ...
            <div class="w-full overflow-hidden ">
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <table ref="tableRef" class="min-w-full divide-y divide-gray-200 text-sm text-left text-gray-600">
