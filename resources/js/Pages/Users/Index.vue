    ...
    <AuthenticatedLayout>
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-200/80 dark:border-slate-700 dark:bg-slate-900 dark:shadow-none">
            <div class="flex items-start justify-between gap-4 pb-5">
                <div>
                    <h1 class="mb-1 text-2xl font-semibold text-slate-900 dark:text-slate-100">Gestión de Usuarios</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">Total: <span class="font-semibold text-slate-700 dark:text-slate-200">{{ users.data.length }}</span> usuarios</p>
                </div>
            ...
            <div class="w-full overflow-hidden">
                <div class="relative overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-950 ">
                    <DataTable :value="users.data" v-model:filters="filters" ref="dt" selectionMode="single"
                        ...
                        class="users-table dark:bg-slate-950">
                        <template #header>
                            ...
                            <template #body="{ data }">
                                <div class="flex flex-wrap items-center justify-center gap-2">
                                    <span v-for="role in data.roles" :key="role.id"
                                        class="inline-flex items-center rounded-full border border-sky-200 bg-sky-100 px-2.5 py-1 text-xs font-semibold text-sky-700 dark:border-sky-800 dark:bg-sky-900/40 dark:text-sky-200">
                                        {{ role.name }}
                                    </span>
                                </div>
                            </template>
                                    ...
                                    <label class="flex items-center cursor-pointer select-none">
                                        <div class="relative">
                                            ...
                                            <div class="block h-6 w-11 rounded-full transition"
                                                :class="rowStates[data.id] ? 'bg-emerald-500 dark:bg-emerald-400' : 'bg-rose-500 dark:bg-rose-400'">
                                            </div>
                                            <div class="absolute left-1 top-1 h-4 w-4 rounded-full bg-white shadow-sm transition"
                                                :class="rowStates[data.id] ? 'translate-x-5' : 'translate-x-0'">
                                            </div>
                                        </div>
                                        <span class="ml-3 text-sm font-medium text-slate-700 dark:text-white">{{ rowStates[data.id] ? 'Activo' : 'Inactivo' }}</span>
                                    </label>
                                </div>
                            </template>
                        </Column>
                        ...
                        <Column v-if="canEdit || canDelete" header="acciones" #body="slotProps"
                            ...
                            <div class="flex items-center justify-center gap-2">
                                <button v-if="canEdit" @click="openModalForm(2, slotProps.data)"
                                    class="inline-flex rounded-lg p-2 transition hover:bg-blue-100 dark:hover:bg-blue-900/30">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="size-5 text-blue-600 dark:text-blue-400">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
                                    </svg>
                                </button>
                                <button v-if="canDelete" @click="openModalDel(slotProps.data)"
                                    class="inline-flex rounded-lg p-2 transition hover:bg-red-100 dark:hover:bg-red-900/30">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="size-5 text-red-600 dark:text-red-400">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="username" v-model="form.username" autocomplete="username" class="block w-full rounded-xl border border-slate-300 bg-white py-2.5 ps-10 pr-3 text-sm text-slate-900 shadow-sm transition placeholder:text-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500" placeholder="user1" />
                            </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="email" v-model="form.email" autocomplete="email" class="block w-full rounded-xl border border-slate-300 bg-white py-2.5 ps-10 pr-3 text-sm text-slate-900 shadow-sm transition placeholder:text-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500" placeholder="user1@gmail.com" />
                            </div>
                            <p v-if="hasError('email')" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ getErrorMessage('email') }}</p>
                        </div>

                        <div>
                            <label for="nombres" class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-200">Nombres</label>
                            <div class="relative">
                                <div class="pointer-events-none absolute inset-y-0 start-0 flex items-center ps-3">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5 text-slate-400 dark:text-slate-500">
                                ...
                                <input type="text" id="nombres" v-model="form.nombres" autocomplete="given-name" class="block w-full rounded-xl border border-slate-300 bg-white py-2.5 ps-10 pr-3 text-sm text-slate-900 shadow-sm transition placeholder:text-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500" placeholder="pablito" />
                            </div>
                            <p v-if="hasError('nombres')" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ getErrorMessage('nombres') }}</p>
                        </div>

                        <div>
                            <label for="apellidos" class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-200">Apellidos</label>
                            <div class="relative">
                                <div class="pointer-events-none absolute inset-y-0 start-0 flex items-center ps-3">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5 text-slate-400 dark:text-slate-500">
                                        <path fill-rule="evenodd" d="M3 3.5A1.5 1.5 0 0 1 4.5 2h6.879a1.5 1.5 0 0 1 1.06.44l4.122 4.12A1.5 1.5 0 0 1 17 7.622V16.5a1.5 1.5 0 0 1-1.5 1.5h-11A1.5 1.5 0 0 1 3 16.5v-13Zm10.857 5.691a.75.75 0 0 0-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 0 0-1.06 1.061l2.5 2.5a.75.75 0 0 0 1.137-.089l4-5.5Z" clip-rule="evenodd" />
                                ...
                                <input type="text" id="apellidos" v-model="form.apellidos" autocomplete="family-name" class="block w-full rounded-xl border border-slate-300 bg-white py-2.5 ps-10 pr-3 text-sm text-slate-900 shadow-sm transition placeholder:text-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500" placeholder="perez gomez" />
                            </div>
                            <p v-if="hasError('apellidos')" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ getErrorMessage('apellidos') }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" id="telefono" v-model="form.telefono" autocomplete="tel" class="block w-full rounded-xl border border-slate-300 bg-white py-2.5 ps-10 pr-3 text-sm text-slate-900 shadow-sm transition placeholder:text-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500" placeholder="77712332" />
                            </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="direccion" v-model="form.direccion" autocomplete="street-address" class="block w-full rounded-xl border border-slate-300 bg-white py-2.5 ps-10 pr-3 text-sm text-slate-900 shadow-sm transition placeholder:text-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500" placeholder="Z. perdidos" />
                            </div>
                            <p v-if="hasError('direccion')" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ getErrorMessage('direccion') }}</p>
                        </div>

                        <div>
                            <label for="rol" class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-200">Rol</label>
                            <select v-model="form.rol" @change="clearError('rol')" :class="{ 'border-red-500': hasError('rol') }" id="rol" class="block w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 shadow-sm transition focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100">
                                <option value="">Seleccionar rol</option>
                                <option v-for="role in roles" :key="role.id" :value="role.name">{{ role.name }}</option>
                            </select>
                            <span v-if="hasError('rol')" class="mt-1 block text-sm text-red-600 dark:text-red-400">{{ getErrorMessage('rol') }}</span>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="password" id="password" v-model="form.password" autocomplete="new-password" class="block w-full rounded-xl border border-slate-300 bg-white py-2.5 ps-10 pr-3 text-sm text-slate-900 shadow-sm transition placeholder:text-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500" placeholder="C0ntras3ñ4" />
                            </div>
                            <p v-if="hasError('password')" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ getErrorMessage('password') }}</p>
                        </div>
                    </div>
            ...
            <div class="bg-slate-50 p-6 dark:bg-slate-950">
                ...
                <div class="flex gap-4 pb-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <p class="text-base font-semibold text-slate-900 dark:text-slate-100">{{ eform.username }} - {{ eform.nombres }} {{ eform.apellidos }}</p>
